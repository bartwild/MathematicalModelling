...
# Definicja miary ryzyka (œrednia ró¿nica Giniego) z u¿yciem zmiennych pomocniczych:
var d {SCENARIOS, SCENARIOS} >= 0;

# Ograniczenia dla zmiennych pomocniczych d
subject to DConstraint {sc1 in SCENARIOS, sc2 in SCENARIOS: sc1 != sc2}:
    d[sc1, sc2] >= profit[sc1] - profit[sc2];

# Definicja ryzyka jako sumy wa¿onej zmiennych pomocniczych
var Risk;
subject to GiniDefinition:
    Risk = sum {sc1 in SCENARIOS, sc2 in SCENARIOS: sc1 != sc2} p[sc1] * p[sc2] * 10*d[sc1, sc2];
...
var security = ExpProfit - lambda * Risk;

# Funkcja celu - chcemy maksymalizowaæ "Security" - miarê bezpieczeñstwa
maximize Security:
   security;
